...
public interface JobQuotaDao extends Dao<JobQuota, Long> {
    ...
    @Query("SELECT new com.jisiben.hrms.domain.dao.bean.Pair(jq.job.company, jq.quota) FROM JobQuota jq where jq.month = :month and (:company is null or jq.job.company = :company)")
